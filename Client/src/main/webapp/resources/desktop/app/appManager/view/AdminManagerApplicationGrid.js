...
Ext.define('Wdesktop.app.AppManager.view.AdminManagerApplicationGrid', {
	extend : 'Ext.grid.Panel',
	alias : "widget.adminManagerApplicationGrid",
				...
				flex : 1
		         ...
		         items: [{
		        	...
		        	handler: function(grid, rowIndex, colIndex) {
		        		...
		        		mainApplication.tools.ajaxTool.PostJsonData('admin/builtInApp/add.json',"["+data.join(",")+"]",function(response,options){
	...
	getApplicationStore : function() {
		return Ext.create('Wdesktop.app.AppManager.store.ApplicationStore',{
			proxy: {
                type: 'ajax',
                url: 'admin/app/getCheckApp.json',
                reader: {
                    type: 'json',
                    root: 'apps'
                }
	...
	onInstallButtonClick:function(){
		var gridPanel = Ext.ComponentQuery.query('adminManagerApplicationGrid')[0];
		var apps = gridPanel.getSelectionModel().getSelection();
		if(apps.length <=0){
			Ext.MessageBox.alert('警告','请选择需要添加为内置的应用.',null,this);
			return;
		}
		var data = [];
